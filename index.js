...
const winCond = () => {
  ...
  const wonDiagPrinc = board.reduce( (acc, cur, currIndex) => {
    if (cur[currIndex] === undefined) return false    // undefined -> não ganhou na diagonal principal
    if (cur[currIndex] !== board[0][0]) return false  // valores diferentes -> não ganhou na diagonal principal
    return acc                                        // enquanto as duas condições acima forem falsas, está certo
  }, true)
  
  if (wonDiagPrinc === true)
    return {
      win: 'dia',
      num: 1,
      player: board[0][0]
  ...
  const wonDiagSec = board.reduce( (acc, cur, currIndex) => {
    const diagInd = board.length-1 - currIndex
    if (cur[currIndex] === undefined) return false    // undefined -> não ganhou na diagonal secundária
    if (cur[diagInd] !== board[0][board.length-1])    // valores diferentes -> não ganhou na diagonal secundária
      return false
    return acc                                        // enquanto as duas condições acima forem falsas, está certo
  }, true)
